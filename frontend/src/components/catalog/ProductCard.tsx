...
export function ProductCard({ product, onSelect, onTryOn, onBuildOutfit }: ProductCardProps) {
  return (
    <Card className="group cursor-pointer overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5">
      <div className="relative" onClick={() => onSelect(product)}>
      ...
      <CardContent className="space-y-2 p-4" onClick={() => onSelect(product)}>
      ...
      <CardFooter className="grid grid-cols-2 gap-2 border-t p-3">
        <Button
          size="sm"
          variant="outline"
          className="text-xs"
